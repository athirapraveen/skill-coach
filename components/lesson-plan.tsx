...
export function LessonPlan({ topics, onToggleResourceComplete, activeTopicId }: LessonPlanProps) {
  // Extract all resources from topics and filter out invalid ones
  const filterInvalidResources = (resources: Resource[]): Resource[] => {
    return resources.filter(resource => {
      // Filter out resources with common validation issues
      
      // Check if URL is missing or empty
      if (!resource.url || resource.url.trim() === '') {
        console.log('Filtering out resource with empty URL:', resource.title);
        return false;
      }
      
      // Filter out resources that have been validated as invalid
      if (resource.url_validated === false && resource.url_status_code) {
        // If it's a 404, 403, 500, or other error status, filter it out
        if (resource.url_status_code >= 400) {
          console.log(`Filtering out resource with error status ${resource.url_status_code}:`, resource.title);
          return false;
        }
      }
      ...
      if (knownBrokenLinks.includes(resource.url)) {
        console.log('Filtering out known broken link:', resource.url);
        return false;
      }
      
      // Filter YouTube videos that may be unavailable
      if ((resource.url.includes('youtube.com') || resource.url.includes('youtu.be')) && 
          resource.url_error && 
          (resource.url_error.includes('video') || 
           resource.url_error.includes('unavailable') || 
           resource.url_error.includes('private'))) {
        console.log('Filtering out unavailable YouTube video:', resource.url);
        return false;
      }
  ...
  const allResources = topics.flatMap(topic => 
    filterInvalidResources(topic.resources).map(resource => ({
      ...resource,
      week: parseInt(topic.id.replace('topic-', '')),
      completed: resource.completed || false // Initialize as not completed if undefined
    }))
  );
  ...
  const resourcesByWeek = allResources.reduce(
    (acc, resource) => {
      const week = resource.week as number;
      if (!acc[week]) {
        acc[week] = [];
      }
    ...
    {} as Record<number, typeof allResources[number][]>
  );
  ...
  const weekCompletionPercentage = (week: number) => {
    ...
    const completedCount = weekResources.filter((r: Resource) => r.completed).length;
  ...
  const getResourcesByTopicForWeek = (week: number) => {
    const weekResources = resourcesByWeek[week as keyof typeof resourcesByWeek] || [];
  ...
  // Helper function to show a warning for potentially invalid URLs
  const getValidationStatus = (resource: Resource) => {
    // If the resource hasn't been validated, show nothing
    if (resource.url_validated === undefined) {
      return null;
    }
    
    // If the resource is valid, show nothing
    if (resource.url_validated === true) {
      return null;
    }
    
    // If the validation is older than 7 days, show a milder warning
    if (resource.url_validated_at) {
      const validationDate = new Date(resource.url_validated_at);
      const daysSinceValidation = Math.floor((new Date().getTime() - validationDate.getTime()) / (1000 * 60 * 60 * 24));
    ...
    // If the resource had a specific error, show details
    if (resource.url_error) {
      return (
        <Badge variant="outline" className="bg-red-50 text-red-700 border-red-200 text-xs ml-2">
          <span className="inline-block w-2 h-2 rounded-full bg-red-500 mr-1"></span>
          {resource.url_error.includes('404') ? 'Page not found' : 
           resource.url_error.includes('video') ? 'Video unavailable' : 
           'May be unavailable'}
        </Badge>
      );
    }
    
    // Generic warning for invalid resources
    return (
      <Badge variant="outline" className="bg-amber-50 text-amber-700 border-amber-200 text-xs ml-2">
        <span className="inline-block w-2 h-2 rounded-full bg-amber-500 mr-1"></span>
        May be unavailable
